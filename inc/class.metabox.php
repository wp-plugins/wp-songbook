...
class songbook_metabox extends songbook_functions {
    ...
    function savesongmeta() {
        ...
        $fields=['files','duration','tempo','writtenin'];
        
        if(!$data)return 1;
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)return 2;
        if(!wp_verify_nonce($data['nonce'],plugin_basename(__FILE__)))return 3;
        
        foreach($fields as $field){
            
            if(is_array($data[$field]) && count($data[$field]<0))$fc='update';
                else $fc='delete';
            if(!$data[$field])$fc='delete';
                else $fc='update';
                
            if($fc==='delete')delete_post_meta (get_the_ID(),$field);
            if($fc==='update')update_post_meta(get_the_ID(),$field,$data[$field]);
