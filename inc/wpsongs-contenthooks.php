...
function songbook_contentfilter($songbook_toedit) {
    if(get_option('songbook_disp_filelistinsong')=='display'&&get_post_meta(get_the_ID(),'songbook_filebox',true)){
...
';
    if($songbook_files[0])$songbook_files_result.='<table class="songbook_songfiles">';
    ...
    if($songbook_files[0])$songbook_files_result.='</table>';
    }
...
if((get_post_type()=='song'&&is_single())||get_the_term_list($post->post_id,'songauthor')){
   ...
   return $songbook_toedit.$sb_files_result.$songbook_editedcontent;
} else {
