...
class songbook_admin extends songbook_functions {
    ...
    function regadminpage() {
        add_submenu_page('edit.php?post_type=song', __('Settings', WPSB_LANGDOM), __('Songbook settings', WPSB_LANGDOM), 'edit_dashboard', 'songbook-settlink', array($this, 'create_adminset'));
    }

    public function create_adminset() {
        if (isset($_GET['reset']) && !isset($_POST['sb_savesets']))$this->defs_all();
            ...
            if (wp_verify_nonce($_POST['_wpnonce'])) {
                ...
                if (isset($_POST['songbook'])) {
                    ...
                    if ($vals['shcdefs_listpageid'] === 'autoaddpage') {
                        ...
                        $title = (!empty($vals['shcdefs_autoadd_pgtitle'])) ? $vals['shcdefs_autoadd_pgtitle'] : __('Songs', WPSB_LANGDOM);
                        if (!get_page_by_title(__($title, WPSB_LANGDOM))) {
                            $post = array(
                                'menu_order' => '5',
...
//          define admin pages
        $pages['pluginfo.php']=__('Plugin info', WPSB_LANGDOM);
        $pages['basic.php']=__('Plugin info', WPSB_LANGDOM);
        $pages['basic.php']=__('Songbook settings', WPSB_LANGDOM);
        $pages['capabilities.php']=__('Capabilities', WPSB_LANGDOM);
//        $pages['modules.php']=__('Plugin modules',WPSB_LANGDOM);
        $pages['songcontent.php']=__('Song content', WPSB_LANGDOM);
        $pages['songlist.php']=__('Song list behavior', WPSB_LANGDOM);
        $pages['language.php']=__('Language', WPSB_LANGDOM);
    ...
    function pluginspagelink($links) {
        $songbook_pluginlinks = array(
            'edit.php?post_type=song&page=songbook-settlink' => __('Settings', WPSB_LANGDOM)
        );
    ...
    function pluginmetalinks($links, $file) {
        if ($file != 'wp-songbook/wordpress-songbook.php')
            return$links;
        $songbook_pluginlinks = array(
            admin_url('edit.php?post_type=song&page=songbook-settlink')=> __('Songbook settings', WPSB_LANGDOM),
            'https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=65SS8NS48FPFQ&lc=CZ&item_name=%c5%a0imon%20Jan%c4%8da&currency_code=CZK&bn=PP%2dDonationsBF%3abtn_donateCC_LG%2egif%3aNonHosted' => __('Donate me', WPSB_LANGDOM)
        );
        foreach (array_keys($songbook_pluginlinks) as $songbook_pluginlinkkey) {
